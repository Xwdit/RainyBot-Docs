...
##
## RainyBot的App消息类，通常代表一个对应实例，此类消息通常由各类App直接发出
##
## @desc:
##     这是RainyBot的App消息类，通常代表一个对应实例，此类消息通常由各类App直接发出
##     此类实例中储存了App消息的原文本，同时允许基于指定APP消息文本来构建一个此类实例
##
...
## 基于指定的App消息文本来手动构造一个AppMessage类的实例
